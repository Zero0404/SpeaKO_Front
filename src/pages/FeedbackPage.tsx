...
export const FeedbackPage: React.FC<FeedbackPageProps> = ({ onComplete }) => {
  ...
  const [feedbackData] = useState({
    overallScore: 87,
    aiComment: '', // 👈 문구를 빈 상태로 비워두어 상세 피드백 안의 텍스트가 깔끔하게 지워지도록 했습니다.
  });

  // 원본 텍스트 및 인식 텍스트 내용 상태 (줄바꿈 \n 포함)
  ...
  return (
    ...
    >
      {/* 🌟 반응형 전체 페이지 래퍼 (고정 1920x1650 대신 max-width 및 유연한 패딩/갭 적용) */}
      <div className="w-full max-w-[1920px] mx-auto px-4 md:px-8 lg:px-[50px] py-[120px] flex flex-col gap-6 box-border">
        ...
        >
          <div className="flex items-center gap-3">
            <h3 className="text-[18px] font-bold text-[#27272A]">평가 결과</h3>
            <span className="glass-icon-box text-[#6366f1] text-[11px] font-semibold px-3 py-1 rounded-full">
              AI 생성
            </span>
          </div>
