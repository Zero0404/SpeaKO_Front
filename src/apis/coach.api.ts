 ...
 * 응답의 scripts[].scriptId는 CoachViewPage의 "실시간 평가받기"(evaluations/record)에도
 * 그대로 넘겨줄 수 있어서, feedback.ts의 scriptId 하드코딩 이슈를 나중에 여기서 해결할 수 있습니다.
 */
...
export async function fetchHighlightTtsAudio(
  ...
  const res = await fetch(`${AUDIO_API_BASE_URL}${HIGHLIGHT_TTS_ENDPOINT}`, {
    method: 'POST',
    headers: {
